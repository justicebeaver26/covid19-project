LIBNAME MYSASLIB "/home/u63797843/sasuser.v94/temp_sas";

/* Importing the dataset */
 proc import datafile="/home/u63797843/sasuser.v94/Semester1/temp_sas/ResearchTask-Team7.csv" /* Importing data */
 out=covid_data
 dbms=csv
 replace;
 getnames=yes;
...
/*The observed difference in means (Females - Males) is approximately -0.461 kg. This indicates that, on average, females weigh slightly less than
males in this dataset. The p-value is 0.5851, so we fail to reject H_0. We do not have enough statistical evidence to assert that the mean
weight is different between females and males i.e., when H_0 is true it would be unlikely to observe a
difference of -0.461 kg and we would expect that most of the mean weight differences fluctuate between -2.092624 and 1.1684276 kg approximately.
The 95% confidence interval for the mean difference, given by the 5th and 97.5th percentiles of the bootstrap distribution, is
approximately (-2.092624, 1.1684276). Since this interval includes 0, it supports the conclusion that
...
histogram weight_pred2 / transparency=0.5 fillattrs=(color=red)
legendlabel="Males";
...
run;
/*The overlapping credible intervals indicate that there is no significant difference in the mean weights between
females and males. The means of these distributions suggest that the average weight for males is slightly higher than that for
females, but the credible intervals overlap considerably, indicating that the difference is not statistically significant.*/

/*************************************PART1 B*************************************/

TITLE ’Linear Regression Analysis’;
/* Fit ordinary least squares (OLS) regression model */
proc reg data=covid_data;
...
TITLE ’Bayesian analysis ’;
proc mcmc data=covid_data outpost=posterior nmc=20000 thin=10 seed=42;
...
/*main analysis*/
/* Logistic Model */
PROC LOGISTIC DATA=covid_data descending ;
...
PROC GAM DATA = covid_data plots = components(clm commonaxes);
MODEL covidsever (EVENT = '1') = spline(weight, df = 3) spline(duration, df = 3) spline(age, df = 3) / dist = binary;
RUN;



/* Importing the GP dataset */
 proc import datafile="/home/u63797843/sasuser.v94/Semester1/temp_sas/ResearchTask_Task4.csv" /* Importing data */
 out=gp
 dbms=csv
 replace;
 getnames=yes;
